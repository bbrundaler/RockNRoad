...
(function(){
  ...
  /* ── Images de marque (22/08) — le logo du bandeau et le favicon vivent ICI,
     ...
     IMG est le SEUL endroit à changer si les fichiers déménagent un jour
     (ex. dans un sous-dossier : IMG = 'img/'). Aujourd'hui le dépôt est plat,
     les images sont à la racine comme tout le reste.

     UN SEUL logo dans le bandeau, la déclinaison à encre crème — et c'est
     volontaire (22/08, correctif après test Bruno). Le chrome est un
     INVARIANT : tokens.css définit --chrome-bg (#161410) dans :root et ne le
     redéfinit JAMAIS dans [data-theme="sombre"] (« seuls les rôles basculent,
     la marque et le chrome ne bougent pas »). La barre est donc en cuir sombre
     dans les deux thèmes. Une bascule clair/sombre ici n'aurait aucun objet,
     et posait le logo à encre sombre sur fond sombre en thème clair : on ne
     voyait plus que le N doré. La déclinaison à encre sombre
     (rocknroadtexte300.png) sert ailleurs — sur le papier : connexion,
     fiche partagée, pages imprimées.

     Le 300 suffit : le bandeau l'affiche à 44px de haut. Le fichier fait
     131px de haut, soit exactement ce qu'exige un écran 3x (44×3=132) —
     inutile de charger le 600, qui pèse presque quatre fois plus pour une
     image présente sur les 13 pages. */
  var IMG        = '';
  var LOGO       = IMG + 'rocknroadtextefondsombre300.png';
  ...
  /* (22/08) Le logo du bandeau n'est plus du texte : c'est l'image de marque.
     Une seule image, à encre crème — le chrome est sombre dans les deux
     thèmes (voir le commentaire de LOGO plus haut). Hauteurs relevées
     34→44 (bureau) et 28→36 (mobile) : Bruno a choisi les plus grandes des
     trois hauteurs maquettées. La barre fait 56px, il reste donc 6px de
     respiration haut et bas — c'est serré et c'est assumé. Si le logo doit
     encore grandir un jour, ce n'est plus cette valeur qu'il faut toucher
     mais la hauteur de .rnrc-nav, et ça se décide à part. */
  +'.rnrc-logo{display:flex;align-items:center;text-decoration:none;flex-shrink:0;}'
  +'.rnrc-logo img{height:44px;width:auto;display:block;}'
  +'@media(max-width:700px){.rnrc-logo img{height:36px;}}'
  ...
  function inject(){
    ...
    logo.href = 'home.html';
    logo.innerHTML = '<img src="' + LOGO + '" alt="RockNRoad">';
